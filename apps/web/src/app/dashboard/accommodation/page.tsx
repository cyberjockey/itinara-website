...
export default async function AccommodationPage(props: { searchParams: Promise<{ query?: string }> }) {
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 mb-20">
            <header className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-12 pt-8">
                <div className="flex-1">
                    <h1 className="text-4xl md:text-5xl font-heading font-bold text-deep-teak mb-2">Find Your Stay</h1>
                    <p className="text-lg text-stone-gray/80">Discover curated accommodations across Indonesia.</p>
                </div>
            </header>

            <div className="mb-12">
                <SearchBar placeholder="Search by destination..." />
            </div>

            {/* Destinations Grid */}
            <section>
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-deep-teak">Destinations with Great Stays</h2>
                </div>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {destinations?.map((destination: Destination) => (
                        <Link href={`/dashboard/accommodation/${destination.id}`} key={destination.id} className="group flex flex-col h-full">
                            <div className="bg-white rounded-3xl overflow-hidden shadow-sm border border-stone-gray/10 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full flex flex-col">
                                ...
                                <div className="p-6 flex flex-col flex-1 relative">
                                    ...
                                    <div className="flex items-center text-sm font-bold text-terracotta group-hover:translate-x-1 transition-transform mt-auto">
                                        Find Stays <span className="ml-1">→</span>
                                    </div>
                                </div>
                            </div>
                        </Link>
